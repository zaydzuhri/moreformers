...
class Head(nn.Module):
    ...
    def forward(self, x, k):
        ...
        # apply attention to key projection
        out = att @ k # (B, T, T) @ (B, T, C) -> (B, T, C)
        if log:
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        k = self.key(x) # (B, T, C)
        out = torch.cat([self.mhff(head(x, k)) for head in self.heads], dim=-1)
        # project and dropout
...
class FeedForward(nn.Module):
    def __init__(self, config):
        super().__init__()
        head_size = config.n_embd // config.n_head
        self.lin1 = nn.Linear(head_size, 4 * head_size, bias=config.bias)
        self.gelu = nn.GELU()
        self.lin2 = nn.Linear(4 * head_size, head_size, bias=config.bias)
        self.dropout = nn.Dropout(config.dropout)
...
class Block(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.mhff = FeedForward(config)
        self.csa = MultiHeadAttention(config, self.mhff)
...
# GPT Model
class LessFormerShare(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
