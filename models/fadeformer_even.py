...
class Head(nn.Module):
    def __init__(self, config, fade=True):
        super().__init__()
        self.fade = fade
        ...
        if log:
            ...
            self.log_file = open('logs/stagger_attention_log.pkl', 'wb') # open the file in write binary mode
...
class MultiHeadAttention(nn.Module):
    def __init__(self, config, fade=True):
        super().__init__()
        self.heads = nn.ModuleList([Head(config, fade) for h in range(config.n_head)])
...
class Block(nn.Module):
    def __init__(self, config, fade=True):
        super().__init__()
        self.fade = fade
        self.csa = MultiHeadAttention(config, fade)
        self.ff = FeedForward(config)
        self.ln1 = LayerNorm(config.n_embd, config.bias)
        self.ln2 = LayerNorm(config.n_embd, config.bias)
    
    def forward(self, x):
        # layer norm, attention, and add half the residual (even token positions)
        out = self.csa(self.ln1(x))
        res = x[:, [i for i in range(x.shape[1]) if i % 2 == 1], :] if self.fade else x
        out = res + out
...
# GPT Model
class FadeFormerEven(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def forward(self, x, targets=None):
        ...
        if targets is not None:
            # if we are training
            # only take target tokens at faded positions, i.e. even * number of faded blocks
            targets = targets[:, [i-1 for i in range(targets.shape[1]+1) if i % (2 ** (self.config.n_layer-2)) == 0 and i != 0]]
