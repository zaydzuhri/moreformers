...
class Head(nn.Module):
    def __init__(self, config):
        ...
        self.local_key = nn.Linear(config.n_embd, head_size)
    ...
    def forward(self, x, k, v):
        ...
        lk = self.local_key(x) # (B, T, C)
        tk = torch.add(k, lk) # (B, T, C)
        tk = tk * 0.5
        # compute attention "affinities", scale, mask, and softmax
        att = q @ tk.transpose(-2, -1) # (B, T, C) @ (B, C, T) -> (B, T, T)
        att = att * C ** (-0.5)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.cat([head(x, k, v) for head in self.heads], dim=-1)
        # project and dropout
