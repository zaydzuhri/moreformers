...
class Head(nn.Module):
    def __init__(self, config, head_num, fade=True):
        super().__init__()
        self.fade = fade
    ...
    def forward(self, x):
        ...
        if self.fade:
            ...
            keep = [x for x in range(T) if x % 2 == self.head_num % 2]
            if T % 2 != 0 and self.head_num % 2 == 0:
                keep = keep[:-1]
...
class MultiHeadAttention(nn.Module):
    def __init__(self, config, fade=True):
        super().__init__()
        self.heads = nn.ModuleList([Head(config, h, fade) for h in range(config.n_head)])
...
# GPT Model
class FadeFormerStagger(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
