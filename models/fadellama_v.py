...
class Block(nn.Module):
    def __init__(self, config, layer_num, fade):
        super().__init__()
        self.fade = fade
        self.T_fade = int(config.ctx_size // (2**layer_num)) if fade else int(config.ctx_size // (2**(config.n_layer-layer_num-1)))
        print(f'Block {layer_num} fade: {fade} fade length: {self.T_fade} tokens')
...
# LLaMA model
class FadeLLaMAV(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.tok_emb = nn.Embedding(config.vocab_size, config.n_embd)
        self.freqs_cis = precompute_freqs_cis(config.n_embd // config.n_head, config.ctx_size * 2)
        self.blocks = nn.ModuleList([Block(config, n, (n < config.n_layer // 2)) for n in range(config.n_layer)])
