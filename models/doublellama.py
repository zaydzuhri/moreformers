...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.request = nn.Linear(config.n_embd, config.n_embd, bias=config.bias)
    ...
    def forward(self, x, freqs_cis, is_training):
        ...
        # double attention
        att2 = r @ out.transpose(2, 3)
        att2 = att2 / math.sqrt(self.head_size)
        att2 = att2.masked_fill(self.tril[:T, :T] == 0, float('-inf'))
        att2 = F.softmax(att2, dim=-1)

        # apply double attention to x
