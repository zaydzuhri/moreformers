...
class Block(nn.Module):
    def __init__(self, config, layer_num, T_fade):
        super().__init__()
        self.T_fade = T_fade
        print(f'Block {layer_num} fade length: {self.T_fade} tokens')
...
# LLaMA model
class FadeLLaMAFFLin(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        last = 256
        n_fade  = 3
        print(f'fade to: {last} tokens')
        # we want to linearly reduce from starting from ctx_size to last over n_fade layers.
        diff = (config.ctx_size - last) / (n_fade)
        # T_fades = [int(config.ctx_size - (diff * n)) if n < n_fade else last for n in range(config.n_layer)]
        T_fades = [last for n in range(config.n_layer)]
        self.blocks = nn.ModuleList([Block(config, n, T_fades[n]) for n in range(config.n_layer)])
