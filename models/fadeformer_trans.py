...
class TransformHalf(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.input_size = config.n_embd * 2 # input size is twice the embedding size
        self.output_size = config.n_embd # output size is equal to the embedding size
    ...
    def forward(self, x):
        ...
        if T % 2 == 1:
            ...
            T += 1
        # concatenate adjacent pairs of tokens
...
class Block(nn.Module):
    def __init__(self, config, fade=True, layer=0):
        ...
        self.halve = TransformHalf(config)
...
# GPT Model
class FadeFormerTrans(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
