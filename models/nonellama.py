...
# Attention with no projections at all
class Attention(nn.Module):
    ...
    def forward(self, x, freqs_cis, is_training):
        B, T, C = x.shape
        xq = x.view(B, T, self.n_head, self.head_size) # (B, T, H, C/H)
        xk = x.view(B, T, self.n_head, self.head_size) # (B, T, H, C/H)

        # apply rotary embeddings
        xq, xk = apply_rotary_emb(xq, xk, freqs_cis)

        # compute attention "affinities", scale, mask, and softmax
        xq = xq.transpose(1, 2) # (B, H, T, C/H)
        xk = xk.transpose(1, 2) # (B, H, T, C/H)
        att = xq @ xk.transpose(2, 3) # (B, H, T, C/H) @ (B, H, C/H, T) -> (B, H, T, T)
        att = att / math.sqrt(self.head_size) # scale
        att = att.masked_fill(self.tril[:T, :T] == 0, float('-inf')) # mask
        att = F.softmax(att, dim=-1) # softmax

        # apply attention to x again and project
        xv = x.view(B, T, self.n_head, self.head_size) # (B, T, H, C/H)
        xv = xv.transpose(1, 2) # (B, H, T, C/H)
        out = att @ xv # (B, H, T, T) @ (B, H, T, C/H) -> (B, H, T, C/H)
