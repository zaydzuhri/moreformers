...
class MultiHeadAttention(nn.Module):
    def __init__(self, config, layer):
        super().__init__()
        self.heads = nn.ModuleList([Head(config, layer) for _ in range(config.n_head)])
        self.proj = nn.Linear(config.n_embd, config.n_embd, bias=config.bias) # linear for dropout
        self.dropout = nn.Dropout(config.dropout)

    def forward(self, x, train):
        # compute and concat heads in parallel
        out = torch.cat([head(x, train) for head in self.heads], dim=-1)
        # project and dropout
...
class Block(nn.Module):
    ...
    def forward(self, x, train):
        ...
        out = self.csa(self.ln1(x), train)
        res = x[:, -out.shape[1]:] if not train else x
        out = res + out
...
class GPTModes(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.dropout)
        self.blocks = nn.ModuleList([Block(config, l) for l in range(config.n_layer)])
