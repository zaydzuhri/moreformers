...
# Multi-query attention with no value projection
class Attention(nn.Module):
    ...
    def forward(self, x, freqs_cis, is_training):
        ...
        if is_training:
            ...
            new_k = new_k.repeat(1, 1, self.n_head, 1)
        ...
        # we take x as value, so no projection needed
...
class FeedForward(nn.Module):
    def __init__(self, config):
        super().__init__()
        hidden_size = (config.n_embd * 4) + (config.n_embd // 2)
