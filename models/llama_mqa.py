...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer('cache_k', torch.zeros((config.batch_size, config.ctx_size, 1, self.head_size)))
        self.register_buffer('cache_v', torch.zeros((config.batch_size, config.ctx_size, 1, self.head_size)))
    ...
    def forward(self, x, freqs_cis, is_training):
        ...
        if is_training:
            k = self.key(x) # (B, T, C/H)
            k = k.view(B, T, 1, self.head_size)
            v = self.value(x) # (B, T, C/H)
            v = v.view(B, T, 1, self.head_size)
        else:
            new_k = self.key(x[:, -1:, :])
            new_k = new_k.view(B, 1, 1, self.head_size)
            new_v = self.value(x[:, -1:, :])
            new_v = new_v.view(B, 1, 1, self.head_size)
        ...
        # duplicate k and v for each head
        k = k.repeat(1, 1, self.n_head, 1)
        v = v.repeat(1, 1, self.n_head, 1)
