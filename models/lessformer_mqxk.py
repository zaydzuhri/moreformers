...
class Head(nn.Module):
    def __init__(self, config, head_num):
        ...
        self.slice = slice(head_num*head_size, (head_num+1)*head_size)
    ...
    def forward(self, x, k):
        ...
        # apply attention to slice of x
        xs = x[:, :, self.slice]
        # element wise multiplication of key and xs
        xk = torch.mul(k, xs)
        out = att @ xk # (B, T, T) @ (B, T, C) -> (B, T, C)
        if log:
...
class MultiHeadAttention(nn.Module):
    def __init__(self, config):
        ...
        self.heads = nn.ModuleList([Head(config, h_num) for h_num in range(config.n_head)])
    ...
    def forward(self, x):
        ...
        k = self.key(x) # (B, T, C)
        out = torch.cat([head(x, k) for head in self.heads], dim=-1)
        # project and dropout
...
# GPT Model
class LessFormerMQXK(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
