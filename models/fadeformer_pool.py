...
class Pool(nn.Module):
    def __init__(self):
        super().__init__()
        self.pool_size = 2 # pool over two adjacent tokens
        self.stride = 2 # reduce context length by a factor of two
    ...
    def forward(self, x):
        ...
        if T % 2 == 1:
            ...
            T += 1
        # reshape input to (B, C, T) for pooling
        x = x.transpose(1, 2)
        # apply max pooling over time dimension
        x = nn.functional.max_pool1d(x, kernel_size=self.pool_size, stride=self.stride, padding=self.padding)
        # reshape output to (B, T//2, C)
        x = x.transpose(1, 2)
        return x
...
# GPT Model
class FadeFormerPool(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
